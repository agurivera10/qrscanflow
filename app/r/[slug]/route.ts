...
async function persistEvents(events: Record<string, unknown>[]) {
  const base = process.env.SUPABASE_URL;
  const secret = process.env.SUPABASE_SECRET_KEY;
  if (!base || !secret || !events[0]?.workspace_id) { console.info("scanflow.events", JSON.stringify(events)); return; }
  try {
...
export async function GET(request: NextRequest, context: { params: Promise<{ slug: string }> }) {
  ...
  const common = {
    ...
    campaign_id: resolution.campaign_id, occurred_at: now, visitor_id: visitorId, session_id: sessionId,
    request_hash: requestHash, country, region, city, timezone,
    latitude: header(request, "x-vercel-ip-latitude"), longitude: header(request, "x-vercel-ip-longitude"),
    device_type: parsed.device, os: parsed.os, browser: parsed.browser, user_agent: userAgent,
    referrer: request.headers.get("referer"),
  };
  const baseMetadata = { slug, unit_token: unitToken, routing_rule_id: routed.ruleId, geo_is_approximate: true, raw_ip_persisted: false };
  await persistEvents([
    { ...common, event_type: "qr.scan", is_exact: true, metadata: baseMetadata },
    { ...common, event_type: "qr.redirect", is_exact: true, metadata: { ...baseMetadata, destination: new URL(routed.url).hostname } },
  ]);

  const response = NextResponse.redirect(routed.url, 302);
