...
async function resolveDestination(slug: string): Promise<Resolution | null> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error("scanflow.resolve_failed", res.status, await res.text());
      return null;
    }
...
async function persistEvents(events: Record<string, unknown>[]) {
  const base = process.env.SUPABASE_URL;
  const headers = supabaseHeaders();
  if (!base || !headers || !events[0]?.workspace_id) return;
  try {
...
export async function GET(request: NextRequest, context: { params: Promise<{ slug: string }> }) {
  ...
  if (!resolution) return new NextResponse("QR not found", { status: 404 });
